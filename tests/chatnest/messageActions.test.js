...
describe('updateLastBotMessage — Parlant mode', () => {
    ...
    test('shows actions only on last container per queryId', () => {
        const widget = document.createElement('div');
        const { row: r1, container: c1, actions: a1 } = buildBotContainer('q1');
        const { row: r2, container: c2, actions: a2 } = buildBotContainer('q1');
    ...
    test('falls back to last overall when no queryIds', () => {
        const widget = document.createElement('div');
        const { row: r1, container: c1, actions: a1 } = buildBotContainer(null);
...
function updateFilePreview(chatnest, files, filePreview) {
    if (files.length === 0) {
        ...
        if (filePreview._objectUrls) {
            filePreview._objectUrls.forEach(url => { try { URL.revokeObjectURL(url); } catch (_) {} });
            filePreview._objectUrls = [];
        }
        filePreview.innerHTML = '';
        return;
    }
    if (filePreview._objectUrls) filePreview._objectUrls.forEach(url => { try { URL.revokeObjectURL(url); } catch (_) {} });
    filePreview._objectUrls = [];
