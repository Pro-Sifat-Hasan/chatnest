...
function clampDimension(value: any, min: number, max: number): string {
    const numValue = parseInt(value, 10);
    return `${Math.min(Math.max(numValue || 0, min), max)}px`;
}

function clampFontSize(size: any): string {
    let numSize;
...
function formatApiEndpoint(endpoint: any): string {
...
export function initConfig(config: any): any {
