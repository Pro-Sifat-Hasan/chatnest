 ...
 */
export function setupCopyButtons(chatnest: any, messageRow: any, text: any) {
    const botCopyBtn = messageRow.querySelector('.bot-copy-btn');
    if (botCopyBtn) {
        botCopyBtn.addEventListener('click', (e: Event) => {
            e.stopPropagation();
            let textToCopy = text;
    ...
    if (userCopyBtn) {
        userCopyBtn.addEventListener('click', (e: Event) => {
            e.stopPropagation();
            let textToCopy = text;
