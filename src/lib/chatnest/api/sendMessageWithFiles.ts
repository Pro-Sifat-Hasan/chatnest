 ...
 */
export async function sendMessageWithFiles(chatnest: any, message: any, files: File[] = []) {
    ...
    try {
        ...
        const imageFiles = files.filter((f: File) => f && f.type && f.type.startsWith('image/'));
        ...
        if (chatnest.supabaseManager?.isReady && !isEmptyResponse(responseText)) {
            chatnest.supabaseManager.saveChatPair(
                ...
                message,
                responseText
            ).catch((err: any) => console.error('[Supabase] save failed:', err));
        }
