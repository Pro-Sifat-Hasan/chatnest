...
export function showNativeForm(chatnest) {
    ...
    chatnest.widget.querySelectorAll('.cn-native-form-overlay').forEach(el => el.remove());
    ...
    // Scroll lock
    chatnest._nativeFormScrollLock = (e) => {
...
function _autocomplete(field) {
    const map = { email: 'email', tel: 'tel', text: 'name', name: 'name' };
    return map[field.name] || map[field.type] || 'on';
}

function _injectStyles(chatnest) {
